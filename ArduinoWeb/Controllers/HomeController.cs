    ...
    public class HomeController : Controller
    {
        ...
        public MedicaoVm MaisRecentes(int relatorioDispositivoId)
        {
            ...
            if (last3.Any())
            {
                ...
                if (co2 != null) { recente.Co2 = co2.ValorLido; }
            }
        ...
        public IActionResult AddLocation(LocalizacaoHandlerVm model)
        {
            if (@ModelState.IsValid)
            {
                ...
                {
                    model.Sucesso = false;
                    model.Mensagem = "Nome já utilizado ";
                }
                else
                {
        ...
        /// <param name="humidadey"></param>
        /// <param name="luz"></param>
        /// <returns></returns>
        public ActionResult PostData(int id, string ip, decimal? temp, decimal? hum, decimal? co2)
        {
            var results = "Sucesso";
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (co2.HasValue)
                    {
                        ...
                        _context.Medicoes.Add(new Medicao
                        {
                            TipoMedicaoId = (int)TipoMedicaoEnum.Co2,
                            RelatorioDispositivoId = device.RelatorioDispositivoId,
                            LocalizacaoId = device.LocalizacaoId,
                            ValorLido = co2.Value,
                            DataMedicao = reported
                        ...
                        (from set in recentSet
                         select new GoogleVizDataTable.Row
                         {
                             c = new List<GoogleVizDataTable.Row.RowValue>
                            {
                                ...
                                new GoogleVizDataTable.Row.RowValue { v = set.Co2String }
                            }
                         }).ToList();
                }
            ...
            var measureSet =
                (from m in _context.Medicoes.Select(m => m).Include(l => l.Localizacao).AsEnumerable()
                 ...
                 select new MedicaoVm
                 {
                     ...
                     Humidade = g.Where(m => m.TipoMedicaoId == 2).Select(r => r.ValorLido).FirstOrDefault(),
                     Co2 = g.Where(m => m.TipoMedicaoId == 3).Select(r => r.ValorLido).FirstOrDefault()
                 }).ToList();
